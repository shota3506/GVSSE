...
class SentenceEncoder(nn.Module):
    ...
    def __init__(self, vocab, encoder_name, d_model, n_layers=None, n_head=None, d_k=None, d_v=None, d_inner=None, eps=1e-7):
        ...
        if encoder_name == 'GRU':
            ...
            self.var = GRU(self.d_word, d_model, n_layers)
        elif encoder_name == 'LSTM':
            assert n_layers
            self.mean = LSTM(self.d_word, d_model, n_layers)
            self.var = LSTM(self.d_word, d_model, n_layers)
            ...
            self.var = TransformerEncoder(self.d_word, d_model, n_layers, n_head, d_k, d_v, d_inner)
        elif encoder_name == 'MaxPooling':
            self.mean = MaxPooling(self.d_word, d_model)
            self.var = MaxPooling(self.d_word, d_model)
        else:
            raise ValueError
